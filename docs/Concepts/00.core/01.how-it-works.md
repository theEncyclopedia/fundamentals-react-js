...
1. The main goal of the `react` library is that in every state changes find a way to minimize the amount of cost to re-render.
2.
3.
4. Diffing algorithm detects a difference between 2 Virtual DOMs
5. Reconciliation find a minimal ways to update the detected difference.
6. Original DOM is updated.
