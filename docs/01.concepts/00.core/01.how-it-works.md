...
2. React is not faster. Its the way more optimized style for developing a frontend apps
3. Workflow of react
  3.1 State changes
  3.2 Diffing algorithm detects a difference between 2 Virtual DOMs
  3.3 Reconciliation find a minimal ways to update the detected difference.
  3.5 The real DOM is updated in the client side.
